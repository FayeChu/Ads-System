		...
		try {
			...
			try {
				...
				for (int i = 2; i < items.size(); i++) {
					...
					if (adRankScore > adWithHighestRank.getAd_score() * 
							adWithHighestRank.getBid()) {
						adWithSecondHighestRank = adWithHighestRank;
						adWithHighestRank = item;
						ad = item.toJSONObject();
					} else if (adRankScore > adWithSecondHighestRank.getAd_score() * 
							adWithSecondHighestRank.getBid()) {
						adWithSecondHighestRank = item;
					}
				}
					
				// calculate cost using second-price bidding
				double secondHighestAdRankScore = adWithSecondHighestRank.getBid() * 
						adWithSecondHighestRank.getAd_score();
